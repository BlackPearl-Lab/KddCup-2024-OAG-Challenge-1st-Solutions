...
class TrainDatasetForReranker(Dataset):
    ...
    def __getitem__(self, item) -> List[BatchEncoding]:
        ...
        for i, passage in enumerate(passages):
            ...
            # passage_inputs = self.mask_pad_token(passage_inputs)
...
class RerankCollator(DataCollatorForSeq2Seq):
    ...
    def mask_pad_token(self,queries):
        ratio = 0.90
        print("ratio:",ratio)
        for q in queries:
            if random.random() > ratio:
                ...
                # 遍历input_ids列表，并以10%的概率将元素替换为2
                masked_input_ids = [0 if random.random() > 0.9 else id for id in input_ids[:-1]]
                masked_input_ids.append(last_token)
                q['input_ids'] = masked_input_ids
        return queries
    ...
    def __call__(self, features, return_tensors='pt'):
        ...
        if labels is not None:
            ...
            features = self.mask_pad_token(features)
