...
all = ['addcite_step1950', 'addcite_step950', 'addbib_addcite_step1950', 'addbib_addcite_step950', 'mistral_addcite_step1950', 'mistral_addcite_step950']
part = [0.15, 0.15, 0.25, 0.25, 0.1, 0.1]
save = []
new_dict = {}
...
for path in all:
    ...
    test = pd.read_pickle('./data/llm_final_title_moreinfo_processtext.pickle')
    test['pred'] = [pred[str(x)] for x in range(len(test))]
    test['pred'] = test['pred'].apply(lambda x : min(1.0, max(0.0, x)))

    with open('./data/submission_example_test.json', 'r') as f:
        sub = json.load(f)
