...
class INDDataSet(Dataset):
    ...
    def get_paper_authors_v2(self,paper_dict,author_name):
        ...
        authors_str = self.get_truncation_text(' # '.join([now_str] + ['org: ' + org + ' names:  ' + '/'.join(name_list) for org, name_list in author_dict.items() if org!=now_org]),500)
        return authors_str
    ...
    def __getitem__(self, index):
        ...
        if sum_len > self.max_source_length - 1000:  # left 500 for the instruction templete
            total_len = 0
            p = 0
...
class IND4EVAL(Dataset):
    def __init__(self, dataset, tokenizer, max_source_length, max_target_length,test_score_file=None):
    ...
    def __getitem__(self, index):
        ...
        len_profile = [len(i) for i in tokenized_profile]
        sum_len = sum(len_profile)
        if sum_len > self.max_source_length - 1000:
            total_len = 0
            p = 0
            while total_len < self.max_source_length - 1000 and p < sum_len:
                total_len += len_profile[p]
